<div class="flex space-between mt-2">
    ...
    <form method="POST" action="/tweety/public/tweets/{{ $tweet->id }}/like">
           @csrf
    ...
    <form method="POST" action="/tweety/public/tweets/{{ $tweet->id }}/like">
       @csrf
       @method ('DELETE')
           <div class="flex items-center mt-1 ml-5 {{$tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500'}}">
                   <svg viewBox="0 0 20 20" class=" mr-1 w-4">
                       <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                           <g class="fill-current">
